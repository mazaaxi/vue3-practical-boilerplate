...
describe('ShopLogic', () => {
  ...
  describe('fetchProducts', () => {
    it('if products has not yet been loaded', async () => {
      ...
      // mock settings
      vi.spyOn(apis, 'getProducts').mockImplementation(async () => expectedProducts)
  ...
  describe('fetchUserCartItems', () => {
    it('if cart items has not yet been loaded', async () => {
      ...
      const { services, apis, stores } = useServiceDependencies()
      // mock settings
      vi.spyOn(apis, 'getCartItems').mockImplementation(async () => expectedCartItems)
      // sign-in user settings
      await services.account.signIn(TaroYamada.email, TaroYamada.password)
    ...
    it('if an error occurs in the API', async () => {
      ...
      const { services, apis, stores } = useServiceDependencies()
      // mock settings
      vi.spyOn(apis, 'getCartItems').mockImplementation(async () => {
        throw expected
      })
      // sign-in user settings
      await services.account.signIn(TaroYamada.email, TaroYamada.password)
  ...
  describe('incrementCartItem', () => {
    it('basic case - add', async () => {
      ...
      // mock settings
      vi.spyOn(apis, 'addCartItems').mockImplementation(async items => {
    ...
    it('basic case - update', async () => {
      ...
      // mock settings
      vi.spyOn(apis, 'updateCartItems').mockImplementation(async items => {
        expect(items).toEqual([
          {
            uid: TaroYamada.id,
            id: response.id,
  ...
  describe('decrementCartItem', () => {
    ...
    it('basic case - remove', async () => {
      ...
      stores.cart.add(oldCartItem1)
      // mock settings
      vi.spyOn(apis, 'removeCartItems').mockImplementation(async cartItemIds => {
        expect(cartItemIds).toEqual([oldCartItem1.id])
        return [response]
      })
      // sign-in user settings
      await services.account.signIn(TaroYamada.email, TaroYamada.password)
  ...
  describe('checkout', () => {
    it('basic case', async () => {
      ...
      // mock settings
      const syp_checkoutCart = vi.spyOn(apis, 'checkoutCart').mockImplementation(async () => true)
      // sign-in user settings
      await services.account.signIn(TaroYamada.email, TaroYamada.password)
      ...
      expect(syp_checkoutCart).toHaveBeenCalledOnce()
      expect(syp_checkoutCart).toHaveReturnedWith(true)
    ...
    it('if an error occurs in the API', async () => {
      ...
      stores.cart.setAll(CartItems())
      // mock settings
      vi.spyOn(apis, 'checkoutCart').mockImplementation(async () => {
        throw expected
      })
      // sign-in user settings
      await services.account.signIn(TaroYamada.email, TaroYamada.password)
